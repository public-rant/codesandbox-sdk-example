...
export function useProjectCreation(): UseProjectCreationReturn {
  ...
  const createProjectWithStream = useCallback(async (name: string): Promise<string> => {
    if (!name.trim()) {
    ...
    return new Promise((resolve, reject) => {
      const eventSource = new EventSource(`/api/projects/create-stream?name=${encodeURIComponent(name.trim())}`);
