...
export function useSandboxPreview(): UseSandboxPreviewReturn {
  const { sandbox, onPortOpen, onPortClose } = useSandboxContext();
  ...
  // Set up port event listeners for preview state management
  useEffect(() => {
    if (!sandbox) return;
  ...
  }, [sandbox, onPortOpen, onPortClose]);
